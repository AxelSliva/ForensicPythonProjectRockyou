...
def G1():
    for letter in alpha_list:
        for index,item in enumerate(letter):
            print(item)
    #x = [random.randint(0,25) for i in range(1000)]
    #x = [for letter in alpha_list]
    #x = [item(0,25)]
    x =[item for i in enumerate(str(round(list_count_letter.count(letter) / count_total_alpha * 100, 1)))]
    n, bins, patches = plt.hist(x, 100, facecolor='b', alpha=0.5)
    plt.xlabel('Alphabet')
    plt.ylabel(u'Pourcentage')
    plt.axis([0, 25, 0, 100])
    plt.grid(True)
    plt.show()
